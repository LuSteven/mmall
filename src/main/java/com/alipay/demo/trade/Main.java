...
public class Main {
    ...
    private void dumpResponse(AlipayResponse response) {
        if (response != null) {
            log.info(format("code:%s, msg:%s", response.getCode(), response.getMsg()));
            if (StringUtils.isNotEmpty(response.getSubCode())) {
                log.info(format("subCode:%s, subMsg:%s", response.getSubCode(),
                        response.getSubMsg()));
            }
    ...
    public void test_trade_precreate() {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = format("/Users/sudo/Desktop/qr-%s.png",
